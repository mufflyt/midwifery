...
# The evidence file is a person-level artifact and gitignored, so it is absent
# on a runner. Every assertion below that reads it used to chk(FALSE) there,
# reporting four defects that had not been demonstrated.
source(file.path(root, "tests", "helper-optional-inputs.R"))
# sha256_of(): the canonical hash used by write_with_provenance(), so the test
# compares like with like rather than reimplementing the digest.
source(file.path(root, "R", "lib", "provenance.R"))
...
# labels, and every flagged row carries exactly one.
{
  if (is.null(ev) || !"county_impact" %in% names(ev)) {
    have_inputs(EV, "T193 county_impact column and label coverage")
  } else {
...
{
  if (is.null(ev)) have_inputs(EV, "T194 reported-count and prose agreement") else {
    # 2026-08-15: was 1163L. That figure was measured against
    # midwives_geocoded.csv, the wrong coordinate base (23.8% GEOID coverage,
    # county_exact 28.5% against a published 98.8%). Rebuilt on
    # midwives_panel_geocoded.csv -- the 16,898-row accepted set -- the same
    # invariant finds 14. The old number was very largely an artefact of the
    # comparison table, not 1,163 real address conflicts.
    chk(nrow(ev) == 14L,
...
{
  if (is.null(ev) || !"county_impact" %in% names(ev)) {
    have_inputs(EV, "T195 impact-class availability")
  } else {
...
# two ZIPs in one state can sit in different counties, and county is the unit of
# every access finding here.
#
# This used to assert `moved > 0`, because 175 live records demonstrated it. The
# geography rebuild dropped the evidence file from 1,163 rows to 14 and none of
# the survivors is same-state-different-county, so that assertion started
# failing -- not because the shortcut became safe, but because this particular
# dataset stopped containing an example.
#
# A contract that only holds while the data happens to exhibit it is not a
# contract. So the demonstration is now SYNTHETIC (it cannot evaporate) and the
# live count is reported alongside it.
{
  # The classifier must be able to express the case at all. If this label is
  # ever collapsed into "same state, fine", the distinction is gone.
  chk(grepl("different_county_same_state", GH),
      "T198a the classifier distinguishes same-state-different-county from benign")

  # Synthetic: same state, different ZIP, different county. It must NOT be
  # classified as harmless.
  cls <- function(cty_r, cty_g, st_r, st_g) {
    if (is.na(cty_r) || is.na(cty_g)) "unresolvable_zip"
    else if (cty_r == cty_g)         "same_county"
    else if (st_r == st_g)           "different_county_same_state"
    else                             "different_state"
  }
  chk(cls("06075", "06001", "CA", "CA") == "different_county_same_state",
      "T198b two ZIPs in one state, different counties, are not treated as benign")
  chk(cls("06075", "06075", "CA", "CA") == "same_county",
      "T198c the same county really is classified benign")
  ...
  if (!is.null(ev) && all(c("county_impact", "practice_state", "geo_state") %in% names(ev))) {
    ...
    cat(sprintf("       live same-STATE disagreements that change county: %d\n", moved))
  }
}

# T199 (adversarial). The evidence file must have been written by a run that
# read the CURRENT inputs. Cycle 18 built a staleness detector and I was still
# fooled by a stale copy of this very file one cycle later.
#
# ASKED THROUGH PROVENANCE, NOT mtime. The original form compared
# file.mtime(EV) against file.mtime(county_base.csv), and that question cannot
# be answered from a filesystem timestamp:
#
#   * git does not preserve mtimes. On a fresh checkout every file carries the
#     checkout time, so the comparison is decided by write order and means
#     nothing -- it would have been a coin flip in CI rather than a check.
#   * EV is gitignored, so on a runner it is absent and the whole assertion
#     silently did not run. A check that cannot fail is not a check, and this
#     file already knows that: see its own header on being fooled by staleness.
#
# The sidecar can answer it. R/03-geography-hierarchy.R writes EV through
# write_with_provenance(inputs = prov_inputs("data/county_base.csv")), which
# records each input path with the SHA256 the producer actually read. If that
# recorded hash still matches the file on disk, the evidence describes the
# inputs in front of us; if it does not, the evidence is stale no matter what
# any timestamp says. That survives a checkout, which is the point.
{
  SC <- paste0(EV, ".provenance.json")
  IN <- "data/county_base.csv"
  if (have_inputs(c(EV, SC, IN), "T199 evidence written against the current inputs")) {
    prov <- tryCatch(jsonlite::read_json(SC), error = function(e) NULL)
    decl <- if (is.null(prov$inputs)) list() else prov$inputs
    rec <- vapply(decl, function(x) as.character(x$path %||% ""), character(1))
    hit <- which(basename(rec) == basename(IN))

    chk(length(hit) == 1L,
        sprintf("T199a the evidence declares %s as an input [declared: %s]",
                basename(IN),
                if (length(rec)) paste(basename(rec), collapse = ", ") else "none"))

    if (length(hit) == 1L) {
      recorded <- as.character(decl[[hit[1]]]$sha256 %||% "")
      current <- sha256_of(IN)
      chk(identical(recorded, current),
          sprintf("T199b the recorded input hash matches the file on disk [recorded %s, current %s]",
                  substr(recorded, 1, 12), substr(current, 1, 12)))
    }
  }
}
