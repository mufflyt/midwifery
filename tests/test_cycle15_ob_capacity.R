...
PROF <- paste(readLines(file.path(root, "R", "10-county-birth-profiles.R"),
                        warn = FALSE), collapse = "\n")
# data/cms_pos_hospital.csv is 30 MB and gitignored, so it is never present on
# a runner. build_ob_hospital_counts() stopifnot()s on it, which killed this
# whole suite -- including the SEMANTIC assertions below, which read source
# text and need no data at all. Those now run everywhere; only the blocks that
# actually touch the POS extract are skipped, loudly and counted.
HAVE_POS <- have_inputs(file.path(root, POS_FILE),
                        "CMS POS hospital counts (BVA and adversarial blocks)")
if (HAVE_POS) {
