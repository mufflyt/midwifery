#!/usr/bin/env python3
# =============================================================================
# BS4 / String Split CABC Accredited Birth Center Master Parser
# =============================================================================
import re
import csv
...
print("=== String Split CABC Birth Center Master Parser ===")

with open(HTML_FILE, "r", encoding="utf-8", errors="ignore") as f:
    text = f.read()

blocks = text.split("Address:")
print(f"Total raw CABC blocks found: {len(blocks)-1}")

birth_centers = []
for i in range(1, len(blocks)):
    # The name is at the end of the previous block
    prev_text = [line.strip() for line in blocks[i-1].split("\n") if line.strip()]
    name = prev_text[-1] if prev_text else ""
    
    # The address is at the beginning of the current block
    curr_text = [line.strip() for line in blocks[i].split("\n") if line.strip()]
    
    addr_lines = []
    for line in curr_text:
        if "Phone:" in line or "Website" in line or "Accredited" in line:
            break
        addr_lines.append(line)
        
    full_addr = ", ".join(addr_lines)
    
    zip_match = re.search(r"\b(\d{5})\b", full_addr)
    zip_code = zip_match.group(1) if zip_match else ""
    
    state_match = re.search(r"\b([A-Z]{2})\b\s*\d{5}", full_addr)
    state = state_match.group(1) if state_match else ""
    
    if len(name) > 3 and not name.startswith("Browse") and not name.startswith("Search") and not name.startswith("How"):
...
print(f"Successfully Extracted {len(birth_centers)} Verified CABC Accredited Freestanding Birth Centers!")
...
for npi, mw in coh.items():
    ...
    for bc in birth_centers:
        ...
        # Match Tier 1: Exact ZIP Match + Street Number Match
