...
> **This cohort cannot be regenerated by re-running the pipeline.** 249 rows
> differ on a clean re-run: contested NPIs go 95 → 188 because the imported
> one-to-one constraint changed on 2026-08-08, and the 156 class-5 rows come
> from a c5guard step that `match_amcb_to_npi.R` does not perform — the frozen
> artifact's manifest names `amcb_npi_crosswalk_c5guard_…` as its source. See
> [`TECHNICAL_APPENDIX_REPRODUCIBILITY.md`](TECHNICAL_APPENDIX_REPRODUCIBILITY.md).
