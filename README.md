...
### The frozen cohort cannot be regenerated by the current pipeline

**Re-running `match_amcb_to_npi.R` unmodified, on the same roster and the same
panel, does not reproduce `amcb_npi_linkage_FROZEN.csv`.** 249 of 22,309 rows
differ. No row matches a different NPI, and the difference decomposes exactly:

| `npi_match_status` | frozen | re-run today | |
|---|---:|---:|---|
| `ambiguous_contested_npi` | 95 | **188** | the one-to-one constraint changed upstream |
| `candidate_class5_held_out_of_cohort` | 156 | **0** | a step this script does not perform |

Neither cause is a change to this repository.

`rank_one_to_one()` is imported from `~/isochrones` and **stopped consuming NPIs
greedily on 2026-08-08**: where two certificants claim one NPI it now quarantines
both rather than awarding it to the higher scorer. And the frozen artifact's own
manifest names `amcb_npi_crosswalk_c5guard_…` as its source — **`FROZEN` is not
this script's output**, it is the base linkage plus a class-5 guard applied
afterwards, which is where all 156 held-out rows come from.

So `16,892` is reproducible only as *base linkage at the freeze commit* + *the
c5guard step* + *the pre-2026-08-08 bijection*. Two of those three live outside
this repository and one of them has since changed. `CONTESTED_RULE=greedy`
restores the old bijection deliberately, and is **not** the default because 37
of the 93 awards it makes are decided by certification number rather than by
evidence.

This is a property of the published numbers, not a defect awaiting repair, and
it belongs in any methods section that quotes them. Full decomposition, and what
the pipeline now records so the claim stays checkable, in
[`docs/TECHNICAL_APPENDIX_REPRODUCIBILITY.md`](docs/TECHNICAL_APPENDIX_REPRODUCIBILITY.md).
